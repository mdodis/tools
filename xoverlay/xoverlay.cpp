...
{
    int c;
    while ((c = getopt(argc, argv, "l:m:c:")) != -1)
    {
        switch (c)
        {
            case 'l':
...
float get_percentage(const char* command, float min, float max)
{
    ...
    return fmap(0, 100, 0.f, 1.f, result);
}
...
void draw(cairo_t *cr, float t) {
    ...
    cairo_set_source_rgba(cr, BAR_COLOR.r, BAR_COLOR.g, BAR_COLOR.b, 1.0);
...
int main(int argc, char** argv) 
{
    ...
    BAR_COLOR = NRGB::from_hexstr(COLOR_ACCENT_INTENSE);
